...
class Pagination
{
        ...
        for ($loop = $start - 1; $loop <= $end; $loop++) {
            ...
            if ($i >= 0) {
                if ($this->currentPage == $loop) {
                    $output .= "<div class=\"profile__pagination-item\">" . $loop . "</div>";
                } else {
                    $n = ($i == 0) ? '' : $i;
    ...
    function getAJAXlink($count, $text)
    {
        ...
        $this->additionalParam = "{'page' : $pageCount, 'type': '$this->typeAjax'}";
        // return "<a href=\"javascript:void(0);\" " . $this->anchorClass . " 
        //         onclick=\"$.post('" . $this->baseURL . "', " . $this->additionalParam . ", function(data){ 
        //                $('#" . $this->contentDiv . "').html(data); }); return false;\"><div class=\"profile__pagination-item profile__pagination-item--no-active\">"
        //     . $text . '</div></a>';
        return "<a href=\"javascript:void(0);\" " . $this->anchorClass . " 
                onclick=\"
                $.ajax({
                    url: '" . $this->baseURL . "',
                    method: 'POST',
                    data: " . $this->additionalParam . ",
                    beforeSend: function() {
                        $('#loading_modal').addClass('loading--open');
                        $('#" . $this->contentDiv . "').get(0).scrollIntoView();

                    },
                    success: function(response) {
                        $('#" . $this->contentDiv . "').html(response);
                ...
                \"><div class=\"profile__pagination-item profile__pagination-item--no-active\">"
            . $text . '</div></a>';
    }
}
